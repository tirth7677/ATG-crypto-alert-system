...
const ActiveAlerts = () => {
  ...
  const fetchActiveAlerts = async () => {
    ...
    setError('');
    try {
      const response = await axios.get('http://localhost:8080/api/v1/alerts/active');
      setAlerts(response.data.data.alerts);
      setTotalAlerts(response.data.data.total);
    } catch (err) {
      setError('Failed to fetch active alerts. Please try again later.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex">
      ...
      <div className="container mx-auto p-6 ml-64">
        <h1 className="text-3xl font-bold text-center mb-6">Active Alerts</h1>
        {loading ? (
          ...
          <>
            <div className="text-center mb-4 text-lg font-medium">
              Total Active Alerts: <span className="font-bold">{totalAlerts}</span>
            </div>
              ...
              <table className="table-auto w-full border-collapse border border-gray-300">
                <thead className="bg-gray-100">
                  <tr>
                    ...
                    <th className="border border-gray-300 px-4 py-2">Already Informed</th>
